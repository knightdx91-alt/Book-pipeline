...
def manifest(p):
    """The MANIFEST.md text for the files currently sitting in completed-books/."""
    lines = ["# Manifest\n",
...
def main(check=False):
    ...
    if check:
        ...
        if bad:
            ...
            return 1
        # MANIFEST is what makes a stale copy *detectable* rather than silently
        # shipped, so it has to be checked, not just written. It drifted once already:
        # a rebuild copied three new EPUBs in and nobody regenerated it, and because
        # this check only ever compared collected-vs-source, nothing noticed. An
        # unverified checksum file is worse than none — it reads as reassurance.
        mf = os.path.join(OUT, "MANIFEST.md")
        current = manifest(p)
        on_disk = open(mf, encoding="utf-8").read() if os.path.exists(mf) else ""
        if on_disk != current:
            print("STALE MANIFEST — completed-books/MANIFEST.md does not describe the")
            print("files next to it. The files may still be fine; the record is not.")
            print("Fix: python3 tools/collect_completed.py")
            return 1
        print("completed-books/ is up to date with every book's current REVISION,")
        print("every print file is profile-free in the right colour space,")
        print("and MANIFEST.md matches the files beside it")
        return 0
    ...
    # --- MANIFEST -----------------------------------------------------------
    open(os.path.join(OUT, "MANIFEST.md"), "w", encoding="utf-8").write(manifest(p))
